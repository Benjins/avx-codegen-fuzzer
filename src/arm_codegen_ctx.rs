...
pub fn generate_arm_codegen_ctx(ctx : &mut ARMSIMDCodegenCtx, intrinsics_by_type : &HashMap<ARMSIMDType, Vec<ARMSIMDIntrinsic>>, all_intrinsic_return_types : &Vec<ARMSIMDType>) {
	...
	let num_node_iterations : usize = 50 + (ctx.rng.rand_size() % 2000);
	let chance_for_zero_node : f32 = match (ctx.rng.rand() % 4) { 0 => 0.00001, 1 => 0.0001, 2 => 0.001, 3 => 0.02, _ => panic!("") };

	// TODO: Meh, could be better placed
	ctx.reuse_node_idx_denom = 16;
	ctx.reuse_node_idx_num = match ctx.rng.rand() % 8 {
		0 => 0,
		1 => 1,
		2 => 2,
		3 => 3,
